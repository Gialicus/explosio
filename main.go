...
func main() {
	...
	cappuccino := builder.Start("Vendita Cappuccino", "Consegna al tavolo", 2).
		WithHuman("Cameriere", "Servizio", 15, 1).
		DependsOn(
			builder.Node("Assemblaggio", "Mix ingredienti", 1).
				CanCrash(0, 5.0).
				DependsOn(
					builder.Node("Preparazione Caffè", "Espresso", 1).
						WithMaterial("Miscela", "Caffè Arabica", 0.02, 18).
						CanCrash(1, 10.0),
					builder.Node("Montatura Latte", "Vapore", 3).
						WithMaterial("Latte", "Intero", 0.01, 150).
						WithHuman("Barista", "Specialista", 20, 1),
				),
		)
