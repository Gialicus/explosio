...
func TestPrintReportTo_SingleActivity(t *testing.T) {
	root := buildActivity("A", "Desc A", 5, nil)
	root.Slack = 0
...
func TestPrintReportTo_NonCritical(t *testing.T) {
	root := buildActivity("B", "Desc B", 2, nil)
	root.Slack = 3
	var buf bytes.Buffer
	PrintReportTo(&buf, root, 0, true, "")
	out := buf.String()
...
func TestPrintReportTo_WithHumanAndMaterial(t *testing.T) {
	root := buildActivity("R", "Root", 1, nil)
	root.Humans = []HumanResource{{Role: "Cameriere", CostPerH: 15, Quantity: 1}}
	root.Materials = []MaterialResource{{Name: "Caffè", UnitCost: 0.02, Quantity: 18}}
	var buf bytes.Buffer
	PrintReportTo(&buf, root, 0, true, "")
	out := buf.String()
	...
	if !strings.Contains(out, "Mat") {
		t.Errorf("output must contain Mat when Materials present: %q", out)
	}
...
func TestPrintReportTo_WithAsset(t *testing.T) {
	root := buildActivity("R", "Root", 1, nil)
	root.Assets = []Asset{{Name: "Macchina", Description: "Espresso", CostPerUse: 0.5, Quantity: 2}}
	var buf bytes.Buffer
	PrintReportTo(&buf, root, 0, true, "")
	out := buf.String()
...
func TestPrintReportTo_Tree(t *testing.T) {
	child := buildActivity("C", "Child", 2, nil)
	child.Slack = 0
	root := buildActivity("R", "Root", 1, []*Activity{child})
	root.Slack = 0
	var buf bytes.Buffer
	PrintReportTo(&buf, root, 0, true, "")
	out := buf.String()
	...
	// Root at level 0, then child with tree markers
...
func TestPrintReport_NoPanic(t *testing.T) {
	root := buildActivity("X", "X", 1, nil)
	PrintReport(root, 0, true, "")
	// smoke test: non deve andare in panic
}
