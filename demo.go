// Package main provides the demo activity tree (home renovation).
package main
...
// BuildDemoTree returns the sample home renovation activity tree.
func BuildDemoTree() *core.Activity {
	...
	designApproved := must(core.NewActivityBuilder().
		WithName("Design approved").
		WithDescription("Design phase complete - checkpoint").
		WithDuration(*unit.NewDuration(0, unit.DurationUnitDay)).
		WithPrice(*unit.NewPrice(0, "EUR")).
		Build())
	...
	homeRenovation.AddActivity(designApproved)
	homeRenovation.AddActivity(kitchen)
	homeRenovation.AddActivity(bathroom)
	installTiles.AddDependsOn(installPipes) // Tiling waits for plumbing (cross-branch CPM)
